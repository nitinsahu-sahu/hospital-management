...
const generateRoutinePDF = async (data, res) => {
  ...
  const addHeader = () => {
    ...
    doc.fillColor(colors.primary)
      ...
      .font('Helvetica-Bold')
      .text(
        'INVESTIGATION(ROUTINE TEST) SUMMARY',
        18,
  ...
  doc.fillColor(colors.lightText)
    .fontSize(10)
    .font('Helvetica')
    .text('Routine Date & Time:', doc.page.width / 2 + 20, titleY, {
      continued: true,
    ...
    .font('Helvetica-Bold')
    .text(` ${formatDateTime(data.routineData?.createdAt)}`, {
      continued: false
    });
  ...
  const hasInvestigations = data.routineData;
  ...
  if (hasInvestigations) {
    ...
    if (data.routineData?.investigations?.length > 0) {
      renderInvestigationTable('Genetic Investigations', data.routineData, data.routineData.investigations);
    }
    yPos += 8;
  }
...
const getGeneticData = async (routineId) => {
  try {
    const routineData = await BloodInvestigation.findById(routineId)
      .populate('createdBy', '-password -__v')

    if (!routineData) {
      throw new Error('Routine test not found');
    }

    const patientId = routineData.patientId;
    ...
    const doctor = routineData.createdBy || null;
...
// Download pdf
exports.routinesPdf = async (req, res) => {
  try {
    const { routineId } = req.params;

    const data = await getGeneticData(routineId);
    ...
    await generateRoutinePDF(data, res);
  } catch (error) {
