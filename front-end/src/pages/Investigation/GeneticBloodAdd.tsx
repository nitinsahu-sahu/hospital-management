...
// console.log("investigationsCustom",investigationsCustom);
    ...
    useEffect(() => {
        if (success) {
            setSuccessMessage('Genetic blood investigations saved successfully!');
            setSelectedInvestigations([]);
            setTimeout(() => {
                setSuccessMessage('');
    ...
    useEffect(() => {
        if (geneticInvestigationError) {
            setError(geneticInvestigationError);
            setTimeout(() => {
                setError('');
                dispatch(clearGeneticInvestigationError());
            }, 5000);
        }
    }, [geneticInvestigationError, dispatch]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const result = await dispatch(createGeneticInvestigation(investigationData) as any);
            ...
            if (result?.type === 'CREATE_GENETIC_INVESTIGATION_SUCCESS') {
                ...
                setError(result.payload || 'Failed to save genetic blood investigations');
                setTimeout(() => setError(''), 5000);
            }
        } catch (error: any) {
            console.error('Error saving genetic blood investigations:', error);
            setError(error?.message || 'Error saving genetic blood investigations');
            setTimeout(() => setError(''), 5000);
        } finally {
    ...
    return (
        <>
            <PageMeta
                title="Add Genetic Blood | Dr. Yogita Verma"
                description="Add new genetic blood investigations"
            />
            <PageBreadcrumb pageTitle="Add Genetic Blood Investigation" />
                ...
                {selectedPatient && (
                    <div className="mt-6 space-y-6">
                        ...
                        <div className="flex justify-end gap-4">
                            ...
                            >
                                {isSubmitting ? 'Saving...' : 'Save Genetic Investigations'}
                            </button>
                        </div>
                    </div>
                )}
                ...
                {!selectedPatient && (
                    <div className="mt-6 p-8 text-center bg-gray-50 dark:bg-gray-800 rounded-lg">
                        <p className="text-gray-500 dark:text-gray-400">
                            Please select a patient to add genetic blood investigations
                        </p>
                    </div>
                )}
            </div>
...
export default GeneticBloodAdd;